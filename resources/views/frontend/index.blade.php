...
@section('content')
    ...
    <header class="relative w-full h-screen">
        ...
        <div class="absolute pt-20 inset-0 z-10 w-full h-full object-cover bg-gradient-to-r from-white to-transparent">
            <div class="max-w-screen-custom h-full px-4 mx-auto flex flex-col justify-center">
                <h1 data-aos="fade-up" data-aos-delay="200" class="text-4xl md:text-5xl font-black max-w-md">Layanan konsultasi bisnis</h1>
                <p data-aos="fade-up" data-aos-delay="400" class="max-w-md mt-5 text-sm md:text-base">Konsultanku adalah platform konsultasi online dan jasa profesional yang telah dipercaya oleh ratusan pengguna jasa di Indonesia</p>
                <div data-aos="fade-up" data-aos-delay="600" class="flex mt-5">
                    <a href="{{ route('frontend.solution') }}" class="w-48 md:w-52 h-12 md:h-14 text-sm rounded-full font-medium transition-all duration-300 hover:bg-blue-500 hover:text-white border-2 border-blue-500 focus:ring-2 focus:ring-blue-300 text-blue-500 flex justify-center items-center">
    ...
    {{-- END : HEADER --}}
    {{-- START : KEUNGGULAN --}}
    <section class="w-full pt-10 md:pt-20  bg-gradient-to-tr from-blue-700 to-blue-900 md:h-96 relative">
        ...
        <div class="w-full h-full relative max-w-screen-custom mx-auto px-4">
            <div class="w-full py-5 md:py-0">
                <h1 data-aos="fade-up" class="text-2xl md:text-3xl font-bold text-center text-zinc-100">Keunggulan konsultanku</h1>
                <p data-aos="fade-up" data-aos-delay="300" class="mt-3 text-center max-w-lg mx-auto text-zinc-100 text-sm md:text-base">Lihat keunggulan yang kami tawarkan kepada anda, sehingga anda tidak ragu lagi dalam memilih kami</p>
            </div>
            {{-- CARD --}}
            <div class="w-full grid grid-cols-1 md:grid-cols-3 gap-5 pt-10 pb-5 md:transform md:translate-y-16">
                @foreach ($privilleges as $privillege)
                    ...
                    <div data-aos="fade-up" data-aos-delay="{{ $delayPrivillege++ }}00" class="w-full h-auto py-10 md:py-0 md:h-80 px-5 bg-white md:shadow-xl shadow-zinc-200 rounded-lg flex flex-col items-center justify-center">
                        ...
                        <p class="text-center text-zinc-500 max-w-md mx-auto lg:px-5 text-sm md:text-base">{{ $privillege->description }}</p>
                    </div>
                @endforeach
            </div>
        </div>
                ...
                <div class="swiper-wrapper">
                    @foreach ($solutions as $solution)
                        ...
                        <div class="swiper-slide">
                            <div data-aos="zoom-in" data-aos-delay="200" class="w-full px-5 py-10 border hover:border-blue-500 hover:text-blue-500 rounded-lg transition-all duration-300">
                                <img src="{{ asset('storage/uploads/solution/'. $solution->icon) }}" class="w-20 mx-auto" alt="">
                                <h1 class="font-semibold text-xl py-5 text-center">{{ $solution->title }}</h1>
                                <p class="text-center text-zinc-500 text-sm md:text-base">{{ $solution->description }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
