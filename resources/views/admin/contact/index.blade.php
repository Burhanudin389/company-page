...
<div class="w-full bg-white rounded-md p-5">
    <div class="flex items-center justify-between mb-5">
        <div class="py-3">
            ...
            <h1 class="text-slate-900 text-xl">Data kontak</h1>
        </div>
        <div class="flex items-center space-x-3">
            <a href="{{ route('contact.create') }}" class="w-40 h-11 rounded-md text-sm font-medium transition-all duration-300 bg-gradient-to-tr from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-700 focus:ring-2 focus:ring-blue-300 text-white active:text-blue-300 flex justify-center items-center">
        ...
        <table class="w-full table-auto rounded-lg overflow-hidden whitespace-nowrap">
            <thead>
                <tr class="text-left uppercase text-slate-500 bg-slate-100">
                    ...
                    <th class="text-xs p-5 font-medium">Nama</th>
                    <th class="text-xs p-5 font-medium">Informasi kontak</th>
                    <th class="text-xs p-5 font-medium">Tipe kontak</th>
            ...
            <tbody class="divide-y">
                @forelse ($contacts as $contact)
                    <tr class="text-left">
                        <td class="text-sm px-5 py-3 text-slate-700">{{ $contact->id }}</td>
                        <td class="text-sm px-5 py-3 text-slate-700">
                            <div class="block w-10 h-10 bg-slate-300 overflow-hidden rounded-full">
                                <img src="{{ asset('storage/uploads/contact/'. $contact->icon) }}" class="w-full h-full object-cover" alt="">
                            </div >
                        </td>
                        <td class="text-sm px-5 py-3 text-slate-700">{{ $contact->name }}</td>
                        <td class="text-sm px-5 py-3 text-slate-700">
                            <p class="w-64 truncate">{{ $contact->contact }}</p>
                        </td>
                        <td class="text-sm px-5 py-3 text-slate-700">{{ $contact->type }}</td>
                        <td class="text-sm px-5 py-3 text-slate-700">
                            <div class="flex space-x-3">
                                <!-- EDIT BUTTON -->
                                <a href="{{ route('contact.edit', $contact->id) }}" class="w-8 h-8 flex items-center justify-center rounded-md transition-all duration-300 bg-yellow-500 hover:bg-yellow-700 focus:ring-2 focus:ring-yellow-300 text-white">
                                    ...
                                    <div x-show="openConfirmDeleteModal" class="w-full h-screen fixed z-50 inset-0 bg-slate-900 bg-opacity-50 p-3 cursor-pointer flex justify-center items-center">
                                        <div x-show="openConfirmDeleteModal" @click.away="openConfirmDeleteModal = false" x-transition x-transition.duration.200ms class="w-full bg-white cursor-default rounded-lg mx-auto max-w-xl overflow-hidden p-8">
                                            ...
                                            <div class="w-full flex space-x-3 justify-end pt-10">
                                                ...
                                                {{-- DELETE BTN --}}
                                                <form action="{{ route('contact.destroy', $contact->id) }}" method="POST" class="w-full md:w-40">
                                                    @csrf
                                                    @method('DELETE')
