            ...
            <div class="py-3 md:pb-5 flex justify-between items-center">
                <div class="py-3 md:py-5 flex justify-between">
                    <div id="breadcrumbs" class="flex items-center space-x-3">
                        ...
                        <a href="{{ route('users.index') }}" class="text-sm font-medium text-blue-500 hover:underline">Pengguna</a>
                        <span class="iconify" data-icon="bx:chevron-right"></span>
                        <p class="text-sm font-medium text-slate-500">Ubah data pengguna</p>
                    </div>
                </div>
            </div>
            {{-- MAIN CONTENT --}}
            <div class="w-full bg-white rounded-md p-5">
                ...
                <div class="w-full mt-5">
                    <div class="pb-3">
                        ...
                        <h1 class="text-slate-900 text-xl">Ubah data pengguna</h1>
                    </div>
                    <form action="{{ route('users.update', $user->id) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-5 pb-5">
                        @csrf
                        @method('PUT')
                        {{-- NAMA PENGGUNA --}}
                        <div class="relative z-0 w-full mt-5">
                            <input value="{{ $user->name }}" placeholder="Masukkan nama pengguna" type="text" name="name" class="@error('name') border-red-500 @enderror border-gray-300 block py-2.5 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 appearance-none outline-none focus:ring-0 focus:border-blue-600 transition-all duration-300 focus:placeholder:text-blue-400"/>
                            @error('name')
                        ...
                        {{-- EMAIL PENGGUNA --}}
                        <div class="relative z-0 w-full mt-5">
                            <input value="{{ $user->email }}" placeholder="Masukkan email pengguna" type="email" name="email" class="@error('email') border-red-500 @enderror border-gray-300 block py-2.5 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 appearance-none outline-none focus:ring-0 focus:border-blue-600 transition-all duration-300 focus:placeholder:text-blue-400"/>
                            @error('email')
                                <span class="error-message text-xs italic text-red-500 mt-0.5">{{ $message }}</span>
                            @enderror
                        </div>
                        {{-- ROLE PENGGUNA --}}
                        <div class="relative z-0 w-full mt-5">
                            <select value="{{ old('role') }}" name="role" id="role" class="@error('role') border-red-500 @enderror border-gray-300 block py-2.5 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                                <option selected disabled>-- Pilih role pengguna</option>
                                <option {{ $user->roles[0]->name === 'admin' ? 'selected' : '' }} value="admin">Admin</option>
                                <option {{ $user->roles[0]->name === 'employee' ? 'selected' : '' }} value="employee">Karyawan</option>
                                <option {{ $user->roles[0]->name === 'guest' ? 'selected' : '' }} value="guest">Pengunjung</option>
                            </select>
                            {{-- <label for="role" class="@error('role') text-red-500 @enderror text-gray-500 peer-focus:font-medium absolute text-sm duration-300 transform -translate-y-6 scale-90 left-2 top-3 z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Tipe user</label> --}}
                            @error('role')
                                <span class="error-message text-xs italic text-red-500 mt-0.5">{{ $message }}</span>
                            @enderror
                        </div>
