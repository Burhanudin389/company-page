            ...
            <div class="py-3 md:pb-5 flex justify-between items-center">
                <div class="py-3 md:py-5 flex justify-between">
                    <div id="breadcrumbs" class="flex items-center space-x-3">
                        ...
                        <p class="text-sm font-medium text-slate-500">Edit data solusi</p>
                    </div>
                </div>
            </div>
            {{-- MAIN CONTENT --}}
            <div class="w-full bg-white rounded-md p-5">
                ...
                <div class="w-full mt-5">
                    <div class="pb-3 mb-10">
                        ...
                        <h1 class="text-slate-900 text-xl">Edit data solusi</h1>
                    </div>
                    <form action="{{ route('solution.update', $solution->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 gap-5 pb-5">
                        @csrf
                        @method('PUT')
                        {{-- ICON --}}
                        <div class="relative z-0 w-full grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
                            <div class="w-full flex space-x-5 items-center bg-slate-100 p-5 rounded-md">
                                ...
                                <img src="{{ asset('storage/uploads/solution/'. $solution->icon) }}" class="w-28 block" alt="" />
                            </div>
                            <div class="w-full">
                        ...
                        <div class="relative z-0 w-full mt-5">
                            <input value="{{ $solution->title }}" placeholder="Masukkan nama solusi" type="text" name="title" class="@error('title') border-red-500 @enderror border-gray-300 block px-3 py-2.5 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 appearance-none outline-none focus:ring-0 focus:border-blue-600 transition-all duration-300 focus:placeholder:text-blue-400"/>
                            @error('title')
                                <span class="error-message text-xs italic text-red-500 mt-0.5">{{ $message }}</span>
                            @enderror
                        </div>
                        {{-- DESCRIPTION --}}
                        <div class="relative z-0 w-full mt-5">
                            <textarea placeholder="Masukkan deskripsi solusi" rows="3" name="description" class="@error('description') border-red-500 @enderror border-gray-300 block px-3 py-2.5 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 appearance-none outline-none focus:ring-0 focus:border-blue-600 transition-all duration-300 focus:placeholder:text-blue-400">{{ $solution->description }}</textarea>
                            @error('description')
                                <span class="error-message text-xs italic text-red-500 mt-0.5">{{ $message }}</span>
                            @enderror
                        </div>
