            ...
            <div class="py-3 md:pb-5 flex justify-between items-center">
                <div class="py-3 md:py-5 flex justify-between">
                    <div id="breadcrumbs" class="flex items-center space-x-3">
                        ...
                        <p class="text-sm font-medium text-slate-500">Tambah data solusi</p>
                    </div>
                </div>
            </div>
            {{-- MAIN CONTENT --}}
            <div class="w-full bg-white rounded-md p-5">
                ...
                <div class="w-full mt-5">
                    <div class="pb-3">
                        ...
                        <h1 class="text-slate-900 text-xl">Tambah data solusi</h1>
                    </div>
                    <form action="{{ route('solution.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 gap-5 pb-5">
                        @csrf
                        {{-- ICON --}}
                        ...
                        <div class="relative z-0 w-full mt-5">
                            <input value="{{ old('title') }}" placeholder="Masukkan nama solusi" type="text" name="title" class="@error('title') border-red-500 @enderror border-gray-300 block px-3 py-2.5 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 appearance-none outline-none focus:ring-0 focus:border-blue-600 transition-all duration-300 focus:placeholder:text-blue-400"/>
                            @error('title')
                                <span class="error-message text-xs italic text-red-500 mt-0.5">{{ $message }}</span>
                            @enderror
                        </div>
