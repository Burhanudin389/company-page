        ...
        <div class="max-w-screen-xl mx-auto">
            ...
            <div class="py-3 md:pb-5 flex justify-between items-center">
                <div class="py-3 md:py-5 flex justify-between">
                    <div id="breadcrumbs" class="flex items-center space-x-3">
                        ...
                        <p class="text-sm font-medium text-slate-500">Tambah data kontak</p>
                    </div>
                </div>
            </div>
            {{-- MAIN CONTENT --}}
            <div class="w-full bg-white rounded-md p-5">
                ...
                <div class="w-full mt-5">
                    <div class="pb-3">
                        <p class="text-sm font-medium text-slate-500">Solusi</p>
                        <h1 class="text-slate-900 text-xl">Tambah data kontak</h1>
                    </div>
                    <form action="{{ route('contact.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-5 pb-5">
                        @csrf
                        {{-- ICON --}}
                        ...
                        <div class="relative z-0 w-full mt-5">
                            <input value="{{ old('name') }}" placeholder="Masukkan nama" type="text" name="name" class="@error('name') border-red-500 @enderror border-gray-300 block px-3 py-2.5 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 appearance-none outline-none focus:ring-0 focus:border-blue-600 transition-all duration-300 focus:placeholder:text-blue-400"/>
                            @error('name')
                                <span class="error-message text-xs italic text-red-500 mt-0.5">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="relative z-0 w-full mt-5 md:col-span-2">
                            <input value="{{ old('contact') }}" placeholder="Masukkan informasi kontak" type="text" name="contact" class="@error('contact') border-red-500 @enderror border-gray-300 block px-3 py-2.5 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 appearance-none outline-none focus:ring-0 focus:border-blue-600 transition-all duration-300 focus:placeholder:text-blue-400"/>
                            @error('contact')
                                <span class="error-message text-xs italic text-red-500 mt-0.5">{{ $message }}</span>
                            @enderror
                        </div>
